...
public class FollowCam : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3 castTarget = target.position + (target.up * cassOffset);
    ...
    //주이공 캐릭터의 이동 로직이 완료된 후 처리하기 위해 LateUpdate 에서 구현
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(target.position
                      +(target.up * targetOffset),
                        transform.position);

        //카메라의 충돌체를 표현하기 위핸 구체를 표시
        Gizmos.color = Color.yellow;
        Gizmos.DrawWireSphere(transform.position, colliderRadius);

        //주인공 캐릭터가 장애물에 가려졌는지를 판단할 레이를 표시
        Gizmos.color = Color.red;
        Gizmos.DrawLine(target.position + (target.up * cassOffset), transform.position);
    }
