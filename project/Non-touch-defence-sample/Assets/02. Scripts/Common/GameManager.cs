...
public class GameManager : MonoBehaviour
{
    ...
    //DataManager 를 저장할 변수
    private DataManager dataManager;

    //public GameData gameData;
    //ScriptableObject 를 연결할 변수
    public GameDataObject gameData;

    //인벤토리의 아이템이 변경됐을 때 발생 시킬 이벤트 정의
    ...
    void LoadGameData()
    {
        ...
        //DataManager 를 통해 파일에 저장된 데이터 불러오기
    ...
    void InventorySetup()
    {
        ...
        {
            //인벤토리 UI 에 있는 Slot개수만큼 반복
            for(int j = 0; j < slots.Length; j++)
            {
                //Slot 하위에 다른 아이템이 있으면 인덱스를 추출
    ...
    public void AddItem(Item item)
    {
        ...
        //아이템이 변경 된 것을 실시간으로 반영 하기 위해 이벤트를 발생시킴
        OnItemChange();
    ...
    //인벤토리에서 아이템을 제거 했을 때 데이터를 갱신하는 함수
    ...
    //일시 정지 버튼 클릭 시 호출할 변수
    public void OnPauseClick()
    {
        //일시 정지 값을 토글시킴
        isPaused = !isPaused;
        //Time Scale 이 0 이면 정, 1이면 정상 속도
        Time.timeScale = (isPaused) ? 0.0f : 1.0f;
        //주인공 객체를 추출
        var playerObj = GameObject.FindGameObjectWithTag("PLAYER");
        //주인공 캐릭터에 추가된 모든 스크립트를 추출함
        var scripts = playerObj.GetComponents<MonoBehaviour>();
        //주인공 캐릭터의 모든 스크립트를 활성/비활성화
