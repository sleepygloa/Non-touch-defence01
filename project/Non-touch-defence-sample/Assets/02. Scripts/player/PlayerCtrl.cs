...
//클래스는 System.Serializable 이라는 어트리뷰트를 명시해야
//인스펙터 뷰에 노출됨
[System.Serializable]
...
public class PlayerCtrl : MonoBehaviour
{
    ...
    //이동 속도 변수(public으로 선언되어 Inspector 에 노출)
    public float moveSpeed = 10.0f;
    //회전 속도 변수
    public float rotSpeed = 80.0f;

    //인스펙터 뷰에 표시할 애니메이션 클래스변수
    ...
    void Start()
    {
        //스크립트가 실행 된 후 처음 수행되는 Start 함수에서 Transform 에 컴포넌트를 할당
        tr = GetComponent<Transform>();

        //Animation 컼포넌트를 변수에 할당
    ...
    void Update()
    {
        ...
        //Transform (이동 방향 * 속도 * 변위 값 * Timel.deltaTime, 기준좌표 )
        ...
        {
            anim.CrossFade(playerAnim.idle.name, 0.3f); //정지 시 Idle 애니메이션aa
        }
    }
}
